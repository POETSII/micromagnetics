{{f:fd_point_update_state_vars_1d.c}}

/* Which buffer should we use for neighbour's magnetisation? Odd or even? */
size_t bufIdx = DEVICESTATE(iteration) & 1;
m_x0_x0plus = DEVICESTATE(m_x_neighbour)[bufIdx][0][1][0];
m_x1_x0plus = DEVICESTATE(m_x_neighbour)[bufIdx][0][1][1];
m_x2_x0plus = DEVICESTATE(m_x_neighbour)[bufIdx][0][1][2];
m_x0_x0minus = DEVICESTATE(m_x_neighbour)[bufIdx][0][0][0];
m_x1_x0minus = DEVICESTATE(m_x_neighbour)[bufIdx][0][0][1];
m_x2_x0minus = DEVICESTATE(m_x_neighbour)[bufIdx][0][0][2];

/* Compute exchange field, multiplied by the time differential dt and other
 * scaling constants. */
h_eff_dt_x[0] = DEVICEPROPERTIES(exchange_coeff_dt) *
    (m_x0_x0plus + m_x0_x0minus);
h_eff_dt_x[1] = DEVICEPROPERTIES(exchange_coeff_dt) *
    (m_x1_x0plus + m_x1_x0minus);
h_eff_dt_x[2] = DEVICEPROPERTIES(exchange_coeff_dt) *
    (m_x2_x0plus + m_x2_x0minus);

/* Stir in a little DMI in a similar way (it's a cross product, and since we're
...
DEVICESTATE(m_x)[0] = DEVICESTATE(m_x)[0] + dm_x[0];
DEVICESTATE(m_x)[1] = DEVICESTATE(m_x)[1] + dm_x[1];
DEVICESTATE(m_x)[2] = DEVICESTATE(m_x)[2] + dm_x[2];
